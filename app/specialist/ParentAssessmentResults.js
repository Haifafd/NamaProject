...
export default function ParentAssessmentResults() {
  ...
  const loadAssessment = async () => {
    ...
    try {
      const data = await getLatestParentAssessment(childId);
      setAssessment(data);
    } catch (error) {
      ...
      <View style={styles.container}>
        ...
        <View style={styles.headerGradient}>
          ...
          <View style={styles.headerRow}>
            ...
            <View style={styles.headerCenter}>
              <Text style={styles.headerTitle}>استمارة ولي الأمر</Text>
            </View>
            <View style={{ width: 38 }} />
          </View>
        </View>
        <View style={styles.emptyState}>
          ...
          <Text style={styles.emptyTitle}>
            لم يقم ولي الأمر بتعبئة الاستمارة بعد
          </Text>
          <Text style={styles.emptySub}>
            سيتم إشعار ولي الأمر عند فتحه التطبيق
          </Text>
        </View>
      </View>
    );
  ...
  const shortNames = {
    ...
    "المهارات اللغوية": "لغوي",
    "المهارات الاجتماعية والانفعالية": "اجتماعي",
    "المهارات الحركية": "حركي",
    "الاعتماد على النفس أو الاستقلالية": "استقلال",
  };
  ...
  const sortedCategories = [...categoryNames].sort(
    (a, b) =>
      (assessment.result.categoryScores[b].percentage || 0) -
      (assessment.result.categoryScores[a].percentage || 0)
  );
  const strengths = sortedCategories
    .filter((c) => assessment.result.categoryScores[c].percentage >= 60)
    .slice(0, 2);
  const weaknesses = sortedCategories
    .filter((c) => assessment.result.categoryScores[c].percentage < 40)
    .slice(0, 2);
      ...
      <View style={styles.headerGradient}>
        ...
        <View style={styles.headerRow}>
          ...
          <View style={styles.headerCenter}>
            <Text style={styles.headerTitle}>استمارة ولي الأمر</Text>
            {childName && (
              <Text style={styles.headerSubtitle}>تقييم {childName}</Text>
            )}
          </View>
          <View style={{ width: 38 }} />
        </View>
      ...
      <ScrollView contentContainerStyle={styles.scrollContent}>
        ...
        <View style={styles.detailCard}>
          ...
          {categoryNames.map((catName) => {
            ...
            const meta = PARENT_CATEGORIES[catName] || {};
            return (
        ...
        <View style={styles.insightsCard}>
          <View style={styles.cardHeader}>
            ...
            <Text style={styles.cardTitle}>نقاط القوة والضعف</Text>
          </View>
          ...
          {strengths.length > 0 && (
            <View style={styles.insightSection}>
              <View style={styles.insightLabel}>
                ...
                />
                <Text
                  style={[styles.insightLabelText, { color: COLORS.SUCCESS }]}
                >
                  نقاط القوة
                </Text>
              </View>
              {strengths.map((cat) => (
                <View key={cat} style={styles.insightItem}>
                  ...
                  <Text style={styles.insightText}>
                    {cat} (
                    {assessment.result.categoryScores[cat].percentage}%)
                  </Text>
                </View>
              ))}
            </View>
          ...
          {weaknesses.length > 0 && (
            <View style={styles.insightSection}>
              <View style={styles.insightLabel}>
                ...
                <Text
                  style={[styles.insightLabelText, { color: COLORS.DANGER }]}
                >
                  بحاجة لدعم
                </Text>
              </View>
          ...
          {strengths.length === 0 && weaknesses.length === 0 && (
            <Text style={styles.balancedText}>
              النتائج متوازنة في معظم الفئات
            </Text>
          )}
        </View>
          ...
          <View style={styles.notesCard}>
            <View style={styles.cardHeader}>
              <Ionicons
                ...
                color={COLORS.PRIMARY_DARK}
              />
              <Text style={styles.cardTitle}>ملاحظات ولي الأمر</Text>
            </View>
...
const styles = StyleSheet.create({
  ...
  insightSection: { marginBottom: 14 },
  insightLabel: {
    flexDirection: "row-reverse",
    alignItems: "center",
    gap: 6,
  ...
  insightLabelText: { fontSize: 12, fontWeight: "700" },
  insightItem: {
    flexDirection: "row-reverse",
    alignItems: "center",
    gap: 8,
